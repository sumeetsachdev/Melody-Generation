...
class MelodyGenerator:
    ...
    def __init__(self, model_path="model-04-0.42.h5"):
...
##            probabilities = self.model.predict(onehot_seed)[0]
            probabilities = self.model.predict(seed)
...
##            print(output_symbol)
    ...
    def _sample_with_temperature(self, probabilites, temperature):
        ...
        """
##        probabilites = np.squeeze(probabilites)
...
##        index = np.random.choice(choices, p=probabilites)
    ...
    def predict_output(self, seed, num_steps):
        seed = seed.split()
        melody = seed
        seed = self._start_symbols + seed
        seed = [self._mappings[symbol] for symbol in seed]
        ...
        for _ in range(50):
            seed = seed[-64:]
            prob = self.model.predict(seed)[0]
            index = np.argmax(prob)
            seed.append(index)
            
            output_symbol = [k for k, v in self._mappings.items() if v == index][0]
##            print(output_symbol)
            if output_symbol == "/":
                break
            melody.append(output_symbol)
...
##            index = np.unravel_index(prob.argmax(), prob.shape)
##            print(index)    
